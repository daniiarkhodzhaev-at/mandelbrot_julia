...
def _mandelbrot_number_of_term(x: float, y: float) -> int:
    return julia_number_of_term(0, 0, x, y)

def calc(mx: float, my: float, dx: float, dy: float,
        cx: float, cy: float,
    ...
    for i in range(WIDTH):
        for j in range(HEIGHT):
            m_points[i][j] = _get_color(_mandelbrot_number_of_term(
                mx - dmx / 2 + dmx * i / HEIGHT, my - dmy / 2 + dmy * j / HEIGHT))
    j_points = [[(0, 0, 0) for _ in range(HEIGHT)] for _ in range(WIDTH)]
    for i in range(WIDTH):
        for j in range(HEIGHT):
            m_points[i][j] = _get_color(_mandelbrot_number_of_term(
                jx - djx / 2 + djx * i / HEIGHT, jy - djy / 2 + djy * j / HEIGHT, cx, cy))
